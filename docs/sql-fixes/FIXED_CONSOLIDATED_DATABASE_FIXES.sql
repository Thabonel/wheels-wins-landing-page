-- FIXED CONSOLIDATED DATABASE FIXES - ALL IN ONE
-- Date: January 31, 2026
-- Combines: RLS Security + Calendar Fix + Performance Optimizations
-- FIXED: Removed CONCURRENTLY to work inside transactions
...
DROP POLICY IF EXISTS "secure_calendar_select" ON public.calendar_events;
DROP POLICY IF EXISTS "secure_calendar_insert" ON public.calendar_events;
DROP POLICY IF EXISTS "secure_calendar_update" ON public.calendar_events;
DROP POLICY IF EXISTS "secure_calendar_delete" ON public.calendar_events;
DROP POLICY IF EXISTS "service_role_calendar_access" ON public.calendar_events;
...
-- =============================================================================
-- PART 3: PERFORMANCE OPTIMIZATIONS (FIXED - No CONCURRENTLY)
-- =============================================================================

BEGIN;

-- Remove duplicate indexes (safe to do inside transaction)
...
-- =============================================================================
-- PART 4: ADD PERFORMANCE INDEXES (Outside transaction)
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_active_budgets
...
-- Test calendar access (should return 0 or more, not error)
SELECT
    COUNT(*) as calendar_events_accessible,
    '✅ CALENDAR ACCESS WORKING' as status
...
FROM pg_indexes
WHERE schemaname = 'public'
  AND (indexname LIKE '%_user_%' OR indexname LIKE '%_upcoming%' OR indexname LIKE '%_active%')
