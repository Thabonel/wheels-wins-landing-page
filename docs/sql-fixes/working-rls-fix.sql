...
CREATE POLICY "expenses_manage_own" ON expenses FOR ALL USING (auth.uid()::text = user_id::text) WITH CHECK (auth.uid()::text = user_id::text);
...
CREATE POLICY "pam_feedback_manage_own" ON pam_feedback FOR ALL USING (auth.uid()::text = user_id::text) WITH CHECK (auth.uid()::text = user_id::text);

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'posts' AND column_name = 'user_id' AND table_schema = 'public') THEN
