...
1. Multiple SQL operations try to acquire locks on different tables in different orders
2. Large DO blocks with multiple table operations execute concurrently
3. RLS policy checks and table alterations happen simultaneously

## Solution

Run the migration in **smaller, sequential chunks** instead of one large script.
...
CREATE TABLE IF NOT EXISTS public.budgets (
    ...
    monthly_limit DECIMAL(10,2) NOT NULL DEFAULT 0 CHECK (monthly_limit >= 0),
...
### Step 4: Enable RLS (No Policies Yet)
```sql
-- Run this fourth, wait for completion
...
### Step 5: Create Policies ONE TABLE AT A TIME
...
**5c. User Subscriptions Policies**
```sql
DO $$
BEGIN
...
## Alternative: Use Transactions with Lock Timeout

If you want to run the entire script, use a transaction with a lock timeout:

```sql
BEGIN;
SET LOCAL lock_timeout = '5s';

-- Your entire migration here

COMMIT;
```

If it times out, it will rollback automatically and you can retry.

## Recommended Approach for Supabase

Use the **Supabase SQL Editor** and run each step manually:
1. Copy Step 1 → Execute → Wait for green checkmark
2. Copy Step 2 → Execute → Wait for green checkmark
3. Continue through Step 8

This ensures no deadlocks and gives you visibility at each stage.
...
3. **Wait and retry**: Sometimes simply waiting 30 seconds and retrying works.

## Prevention

- ✅ Run migrations during low-traffic periods
- ✅ Execute steps sequentially, not all at once
- ✅ Use shorter lock timeouts
- ✅ Avoid DO blocks that touch multiple tables
- ✅ Create indexes after data insertion, not during
