...
BEGIN
    ...
    SELECT
        ...
        CASE WHEN tables.rowsecurity THEN '✅ ENABLED' ELSE '❌ DISABLED' END as rls_enabled,
    ...
    ) policy_stats ON tables.tablename = policy_stats.tablename AND tables.schemaname = policy_stats.schemaname
    WHERE tables.schemaname = 'public'
      AND tables.tablename IN ('profiles', 'user_settings', 'pam_conversations', 'pam_messages', 'expenses', 'budgets', 'trips')
    ORDER BY
        CASE
...
CREATE POLICY "Only admins can manage notifications" ON admin_notification_preferences
  FOR ALL USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND (preferences->>'role' = 'admin' OR email LIKE '%@wheelsandwins.com')));
...
CREATE POLICY "Admins can view notification logs" ON admin_notification_log
  FOR SELECT USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND (preferences->>'role' = 'admin' OR email LIKE '%@wheelsandwins.com')));

DROP POLICY IF EXISTS "System can insert notification logs" ON admin_notification_log;
CREATE POLICY "System can insert notification logs" ON admin_notification_log FOR INSERT WITH CHECK (true);
