...
CREATE POLICY "Users can view own settings" ON user_settings FOR SELECT USING ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users can update own settings" ON user_settings;
CREATE POLICY "Users can update own settings" ON user_settings FOR UPDATE USING ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users can insert own settings" ON user_settings;
CREATE POLICY "Users can insert own settings" ON user_settings FOR INSERT WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users can delete own settings" ON user_settings;
CREATE POLICY "Users can delete own settings" ON user_settings FOR DELETE USING ((select auth.uid()) = user_id);
...
CREATE POLICY "Users can manage own conversations" ON pam_conversations FOR ALL USING ((select auth.uid()) = user_id);

-- PAM feedback
DROP POLICY IF EXISTS "Users can manage own feedback" ON pam_feedback;
CREATE POLICY "Users can manage own feedback" ON pam_feedback FOR ALL USING ((select auth.uid()) = user_id);
...
CREATE POLICY "Users can insert own posts" ON posts FOR INSERT WITH CHECK ((select auth.uid()) = user_id);
...
CREATE POLICY "Users can manage own medical records" ON medical_records FOR ALL USING ((select auth.uid()) = user_id);

-- Medical medications
DROP POLICY IF EXISTS "Users can manage own medications" ON medical_medications;
CREATE POLICY "Users can manage own medications" ON medical_medications FOR ALL USING ((select auth.uid()) = user_id);

-- Medical emergency info
DROP POLICY IF EXISTS "Users can manage own emergency info" ON medical_emergency_info;
CREATE POLICY "Users can manage own emergency info" ON medical_emergency_info FOR ALL USING ((select auth.uid()) = user_id);

-- User subscriptions
DROP POLICY IF EXISTS "Users can view own subscriptions" ON user_subscriptions;
CREATE POLICY "Users can view own subscriptions" ON user_subscriptions FOR SELECT USING ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users can insert own subscriptions" ON user_subscriptions;
CREATE POLICY "Users can insert own subscriptions" ON user_subscriptions FOR INSERT WITH CHECK ((select auth.uid()) = user_id);

DROP POLICY IF EXISTS "Users can update own subscriptions" ON user_subscriptions;
CREATE POLICY "Users can update own subscriptions" ON user_subscriptions FOR UPDATE USING ((select auth.uid()) = user_id);

-- Income entries
DROP POLICY IF EXISTS "Users can manage own income" ON income_entries;
CREATE POLICY "Users can manage own income" ON income_entries FOR ALL USING ((select auth.uid()) = user_id);

-- Budgets
DROP POLICY IF EXISTS "Users can manage own budgets" ON budgets;
CREATE POLICY "Users can manage own budgets" ON budgets FOR ALL USING ((select auth.uid()) = user_id);

-- Function to optimize remaining policies
