    ...
    <div className="space-y-8">
      ...
      <div className="bg-blue-50 p-6 rounded-lg">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2">Distance: {distanceRange[0]} km</label>
            <Slider defaultValue={[100]} max={500} step={10} value={distanceRange} onValueChange={setDistanceRange} />
          </div>

          <div className="flex items-center justify-between">
            <label className="text-sm font-medium">Meet Halfway</label>
            <Switch checked={meetHalfway} onCheckedChange={setMeetHalfway} />
          </div>
        </div>
      </div>
      ...
      <div>
        <h3 className="text-xl font-semibold mb-6">{filteredListings.length} Available Items</h3>
            ...
            {filteredListings.map((listing) => (
              <Card key={listing.id} className="overflow-hidden flex flex-col h-full">
                ...
                <CardContent className="pb-2 flex-grow">
                  <p className="text-sm text-gray-700 line-clamp-2 mb-2">{listing.description}</p>
                  <div className="flex items-center text-sm text-gray-600">
                    <MapPin size={16} className="mr-1" /> {listing.distance} km away
                    {meetHalfway && <Badge className="ml-2 text-xs bg-blue-400">Can meet halfway</Badge>}
                  </div>
                ...
                <CardFooter className="pt-2">
                  <Button className="w-full" onClick={() => handleContactSeller(listing.id)}>
                    Contact Seller
                  </Button>
                </CardFooter>
              </Card>
              ...
              <Card key={listing.id} className="overflow-hidden">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="flex-grow p-4">
                    ...
                    <p className="text-sm text-gray-700 my-2">{listing.description}</p>
                    <div className="flex items-center text-sm text-gray-600">
                      <MapPin size={16} className="mr-1" /> {listing.distance} km away
                      {meetHalfway && <Badge className="ml-2 text-xs bg-blue-400">Can meet halfway</Badge>}
                    </div>
                    ...
                    <Button onClick={() => handleContactSeller(listing.id)} size="sm">
                      Contact Seller
                    </Button>
                  </div>
                </div>
      ...
      <Card className="bg-gray-50 border-dashed border-2 border-gray-300 p-6 text-center">
        <CardContent>
          ...
          <p className="text-gray-700 mb-4">List your items securely in the marketplace for other travelers to find.</p>
          <Button>
