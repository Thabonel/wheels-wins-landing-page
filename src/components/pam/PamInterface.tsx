...
import { Send, Mic, MicOff, Volume2, VolumeX, Loader2, MapPin } from 'lucide-react';
...
export const PamInterface: React.FC<PamInterfaceProps> = ({ className, onClose }) => {
  ...
  const { location, getLocationForQuery } = useLocationContext();
  ...
  const handleSend = async () => {
    ...
    try {
      // Get location context only if needed for this query
      const locationData = getLocationForQuery(userMessage.content);
      
      // Build context with smart location inclusion
      const context: any = {
        conversation_history: messages.slice(-5), // Last 5 messages for context
        user_preferences: settings
      };
      
      // Only add location if query needs it
      if (locationData) {
      ...
      const result = await pamAgenticService.planAndExecute(userMessage.content, context);
            ...
            <div className="flex items-center space-x-2">
              ...
              {location && (
                <div className="flex items-center text-xs text-gray-400 dark:text-gray-500">
                  ...
                  <span>{location.city || 'Location ready'}</span>
                </div>
              )}
            </div>
          </div>
