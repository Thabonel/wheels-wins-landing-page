...
import { Menu, X, List, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function Trips() {
  ...
  // Feature toggle for new trip planner
  const [useNewTripPlanner, setUseNewTripPlanner] = useState(() => {
    return import.meta.env.VITE_USE_NEW_TRIP_PLANNER === 'true';
  });
  ...
  // Handle saving trips from the new trip planner
  ...
  // Handle going back to Wheels section
  const handleBack = () => {
    navigate('/wheels');
  };
  ...
  // Render new trip planner if feature flag is enabled
  if (useNewTripPlanner) {
    return (
      <div className="fixed inset-0 bg-gray-100">
        ...
        <div className="absolute bottom-4 left-4 z-50">
          <Button
            ...
            onClick={() => setUseNewTripPlanner(false)}
          >
            <ToggleLeft className="h-4 w-4 mr-2" />
            Switch to Legacy Planner
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-gray-100">
      ...
      {/* Feature toggle button for legacy planner */}
      <div className="absolute bottom-4 left-4 z-50">
        <Button
          variant="outline"
          size="sm"
          className="bg-white/90 backdrop-blur-sm hover:bg-white border shadow-md"
          onClick={() => setUseNewTripPlanner(true)}
        >
