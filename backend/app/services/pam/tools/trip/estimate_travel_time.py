...
async def estimate_travel_time(
    ...
    **kwargs
    ...
    try:
        ...
        distance = distance_miles or 500.0  # Default estimate
        ...
        logger.info(f"Estimated travel time from {origin} to {destination}: {total_hours:.1f} hours for user {user_id}")
        ...
        return {
            ...
            "break_hours": round(break_time_hours if include_breaks else 0, 1),
            "meal_hours": round(meal_time_hours if include_breaks else 0, 1),
            "total_hours": round(total_hours, 1),
            "suggested_days": days + (1 if remaining_hours > 0 else 0),
            "breaks_recommended": num_breaks if include_breaks else 0,
            "meals_recommended": num_meals if include_breaks else 0,
